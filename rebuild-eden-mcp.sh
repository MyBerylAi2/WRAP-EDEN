...
if [ ! -f package.json ]; then
    [ -f package.json ] || echo '{"name":"mcp-backend","version":"1.0.0"}' > package.json
    bun install @anthropic-ai/sdk @modelcontextprotocol/sdk ssh2 > /dev/null 2>&1
fi
cat > index.js << 'SSHEOF'
...
if [ ! -f package.json ]; then
    [ -f package.json ] || echo '{"name":"mcp-backend","version":"1.0.0"}' > package.json
    bun install @modelcontextprotocol/sdk glob > /dev/null 2>&1
fi
...
# 2C: Playwright-MCP
echo -e "${YELLOW}  [2C] Playwright-MCP (deferred — install separately)...${NC}"
echo -e "${GREEN}  ✅ Playwright-MCP (7 tools — run 'cd ~/mcp-servers/playwright-mcp && bun install @anthropic-ai/mcp-server-playwright playwright && npx playwright install chromium' to activate)${NC}"
...
if [ ! -f package.json ]; then
    [ -f package.json ] || echo '{"name":"mcp-backend","version":"1.0.0"}' > package.json
    bun install @modelcontextprotocol/sdk axios > /dev/null 2>&1
fi
...
[ -f package.json ] || echo '{"name":"mcp-backend","version":"1.0.0"}' > package.json
bun install @modelcontextprotocol/sdk @gradio/client node-fetch > /dev/null 2>&1
...
const EDEN_TOOLS = [
    ...
    { name: "eden_space_wake", description: "Wake up sleeping HuggingFace Space (T4 Medium, $0.45/hr — upgrade to A10G for 4D avatar testing)",
      inputSchema: { type: "object", properties: {} }
    },
    // QUALITY VALIDATION
...
cd "$MCP_ROOT/eve-mcp"
[ -f package.json ] || echo '{"name":"mcp-backend","version":"1.0.0"}' > package.json
bun install @modelcontextprotocol/sdk > /dev/null 2>&1
...
# dotenv support
bun install dotenv > /dev/null 2>&1
...
echo -e "${YELLOW}║    cd ~/mcp-servers/gateway && node src/server.js                     ║${NC}"
echo -e "${YELLOW}║                                                              ║${NC}"
...
echo -e "${YELLOW}╚══════════════════════════════════════════════════════════════╝${NC}"
echo ""
echo -e "${GREEN}🔱 Eden MCP Gateway ready. Run 'cd ~/mcp-servers/gateway && node src/server.js' to launch.${NC}"
